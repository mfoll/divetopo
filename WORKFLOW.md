...
7. La cote a 0 m est interpolee en polygone continu. HYSCORES et RGE ALTI sont fusionnes sans combler les lacunes internes par une couleur arbitraire. Une interpolation locale peut etre activee explicitement pour le seul maillage 3D statique; elle intervient apres l'extraction des isobathes et ne modifie jamais les rasters, les plans 2D ni le paquet interactif.
...
Le champ d'altitude compte au maximum 257 sommets sur son plus grand axe. Les
deux textures sont derivees des rasters de mise au point et limitees a
`2048 px` sur leur plus grand cote; elles ne reutilisent pas le GeoTIFF de
contexte propre aux perspectives 3D statiques.
...
- `plan_max_depth_m` et `interactive_max_depth_m` : limites optionnelles, inferieures ou egales a `max_depth_m`, propres respectivement aux plans 2D et au paquet interactif. Elles sont reservees a un defaut de couverture source documente, pas a un recadrage esthetique. Pointe au Sel utilise `20 m` pour ces deux supports, car la marge sud-ouest du raster HYSCORES contient des facettes profondes triangulees peu fiables; la 3D statique conserve `max_depth_m: 40` dans un cadrage de `800 m` centre 50 m plus au nord.
- `relief_mesh_gap_fill_max_area_m2` : seuil physique optionnel, desactive par defaut, pour interpoler uniquement de tres petites lacunes marines fermees dans le maillage 3D statique. Une composante touchant un bord, la terre ou depassant ce seuil reste invalide. Pointe au Sel fixe `64 m²`; le rendu courant remplit 30 cellules, soit `19,2 m²`, a partir de leur pourtour valide. L'operation est appliquee apres l'extraction des contours et avant la rotation de la camera, de sorte que les isobathes et tous les autres supports restent source-fideles.
...
Les variantes orthophoto sont des sorties supplementaires. En 2D, la texture remplace le fond topographique a l'interieur du masque terrestre et, lorsqu'elle est configuree, dans la tranche marine peu profonde. En 3D, l'orthophoto et son masque alpha georeference sont alignes sur le MNT, puis tournes, recadres et reechantillonnes en parallele du maillage. Le masque alpha devient nul lorsque la profondeur de controle atteint `imagery_sea_max_depth_m`; une bande calculee seulement par distance a la cote ne peut donc pas prolonger l'image en mer profonde. Les lacunes des rasters restent invalides et recoivent un fond neutre, au lieu d'etre transformees en faux fond marin a `max_depth_m`, sauf la petite interpolation de maillage statique explicitement bornee par `relief_mesh_gap_fill_max_area_m2`.
