...
REGION_SLUG = "alpes-maritimes"


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a local Alpes-Maritimes site package"
    )
    ...
    if config.get("region") != REGION_SLUG:
        raise ValueError(
            "The Alpes-Maritimes pipeline requires "
    ...
    if args.check:
        print(
            "Configuration and Alpes-Maritimes source rasters are valid: "
            f"{config_path}"
        )
        return 0
