...
from cartography.config import paths_for, region_manifest, validate_config
...
def marker_wgs84(
    marker: list[float],
    projected_crs: str = "EPSG:32740",
) -> tuple[float, float]:
    projected = osr.SpatialReference()
    projected.ImportFromEPSG(int(projected_crs.split(":", 1)[1]))
