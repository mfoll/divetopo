...
from compose_site_plate import _plate_paths, compose, font, format_dms
...
class PlateConfigurationTests(unittest.TestCase):
    def test_default_paths_are_resolved_without_a_paths_object(self) -> None:
        ...
        self.assertEqual(plan, ROOT / "outputs" / "new-site-topobathy-2d.jpg")
        self.assertEqual(relief, ROOT / "outputs" / "new-site-topobathy-3d.jpg")
        self.assertEqual(locator, ROOT / "outputs" / "new-site-locator-reunion.jpg")
        self.assertEqual(
            output,
    ...
    def test_nonstandard_canvas_width_is_rejected_before_inputs_are_opened(self) -> None:
        ...
        with patch("compose_site_plate.validate_config") as validate:
    ...
    def test_missing_input_error_names_the_file_and_prerequisite(self) -> None:
        missing = ROOT / "outputs" / "definitely-missing-test-image.jpg"
        resolved = (missing, Path("unused"), Path("unused"), Path("unused"))
        with (
            patch("compose_site_plate.validate_config"),
            patch("compose_site_plate._plate_paths", return_value=resolved),
        ):
            with self.assertRaisesRegex(
                FileNotFoundError,
    ...
    def test_font_error_includes_path_and_face_index(self) -> None:
        with patch("compose_site_plate.ImageFont.truetype", side_effect=OSError):
            with self.assertRaisesRegex(
                RuntimeError,
